...
def astronaut_search(args: dict):
...
def astronaut_get(variable, value):
    df = pd.read_json('data/astronaut.json')
...
def astronaut_update(name, value, rq: request):
    x, y = astronaut_delete(name, value)
    if y == 404:
        return {"error": f"Entry not found:  {name} == {value} , cannot delete it"}, 404
    return astronaut_post(rq)
